...
def population_stability_index(reference: pd.Series, current: pd.Series, bins: int = 10) -> float:
    ...
    return float(((current_share - reference_share) * (current_share / reference_share).apply(__import__("math").log)).sum())
...
def run() -> None:
    ...
    for column in FEATURE_COLUMNS:
        if pd.api.types.is_numeric_dtype(reference[column]):
            rows.append({"feature": column, "psi": population_stability_index(reference[column].dropna(), current[column].dropna())})
